...
committo::Json::Json(std::string file) {
  ...
  if (load.is_open()) {
    ...
    pessum::Log(pessum::WARNING, "File \"%s\" does not exist",
                "committo::Json::Json(std::string file)", file.c_str());
  }
}

committo::Json::~Json() { data_set.clear(); }
...
std::string committo::Json::String(int level) {
  std::string str = "";
  if (is_set == true) {
